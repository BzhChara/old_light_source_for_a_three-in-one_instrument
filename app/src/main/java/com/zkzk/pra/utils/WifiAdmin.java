...
//       return mWifiManager.disconnect();		//jemen：这个函数不可调用，否则将会导致再次连接wifi时失败。
    } 
	...
	public boolean Connect(String SSID, String Password,WifiCipherType type) {
		...
		for (WifiConfiguration c0 : mWifiManager.getConfiguredNetworks()) {
			if (c0.networkId == netID) {
				boolean bRet = mWifiManager.enableNetwork(c0.networkId, true);
			} else {
	...
	public boolean Connect(WifiConfiguration wf) {
		...
		for (WifiConfiguration c0 : mWifiManager.getConfiguredNetworks()) {
			Log.d("jemen","connect,c0.networkId="+c0.networkId+",netID="+netID);
			if (c0.networkId == netID) {
				boolean bRet = mWifiManager.enableNetwork(c0.networkId, true);
			} else {
				mWifiManager.enableNetwork(c0.networkId, false);
    ...
    public void rmConfiguration(int wifiItemId) {
    	 mWifiManager.removeNetwork(wifiItemId);
    	 mWifiManager.saveConfiguration();
    };
...
//		LogUtil.d("ssid="+SSID+"PS="+Password+"type="+Type);
