...
public class PhotometerSpecimenDialog extends BaseDialog implements android.view.View.OnClickListener {
	...
	public PhotometerSpecimenDialog(Activity context, PhotometerData data) {
		super(context, R.style.dialog);
		this.context = context;
		this.data = data;
	}
	public PhotometerSpecimenDialog(Activity context, PhotometerData data,ICallback callback) {
		super(context, R.style.dialog);
		this.context = context;
		this.data = data;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initView();
		initValue();
		if (context instanceof PhotometerActivity) { // 样品信息设置null == data.getResult()
			setListeners();
		}
	}
...
//		tvReferenceBasis.append(Params.GB);
		tvReferenceBasis.setVisibility(View.GONE);
	...
	private void initValue() {
		...
		// map = mapView.getMap();
		if (context instanceof PhotometerActivity) { // 样品信息输入,之前使用的null==data.getResult();
	...
	public void onClick(View v) {
		...
		switch (v.getId()) {
		case R.id.bt_save:
			if (context instanceof PhotometerActivity) {
				// MyApplication.getApp().addData(data);
				if(!TextUtils.isEmpty(etSn.getText())){
...
//			LogUtil.d("teaPicker="+teaPickerView+",  v="+v);
			specimenPickerView.showAsDropDown(v, 0, -200);
			break;
